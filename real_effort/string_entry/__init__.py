...
doc = """
Type strings of special characters (/ \\ ) ( _ < and space).
Real-effort task based on Gill & Prowse (2012).
"""


class C(BaseConstants):
    NAME_IN_URL = "string_entry"
...
def creating_session(subsession: Subsession):
    ...
    game_cfg = get_game_config("string_entry")
    defaults = dict(
        retry_delay=1.0,
        puzzle_delay=1.0,
...
def gen_puzzle(player: Player) -> Puzzle:
    fields = string_entry.generate_puzzle_fields(
...
def enc_puzzle(puzzle: Puzzle):
    image = string_entry.render_image(puzzle)
...
def play_game(player: Player, msg: dict):
    ...
    if msg_type == "answer":
        ...
        current.is_correct = string_entry.is_correct(answer, current)
        current.response_timestamp = now
        current.attempts += 1
...
class Game(Page):
    ...
    def vars_for_template(player: Player):
        ...
        return dict(
            ...
            input_type=string_entry.INPUT_TYPE,
            placeholder=string_entry.INPUT_HINT,
