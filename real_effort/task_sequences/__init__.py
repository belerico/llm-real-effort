...
next number. Uses WebSocket live_method with a Question ExtraModel, matching
the same architecture as count_numbers and task_decoding.
"""
...
def load_questions():
    ...
    if _questions_cache is None:
        f = open("task_sequences/sequences.csv", encoding="utf-8-sig")
        _questions_cache = list(csv.DictReader(f))
        f.close()
...
def creating_session(subsession: Subsession):
    ...
    game_cfg = get_game_config("task_sequences")
    defaults = dict(
        retry_delay=1.0,
        puzzle_delay=1.0,
    ...
    # session.config (editable per-session in the oTree admin) wins over the
    # YAML; the YAML still supplies per-game params absent from session.config.
    for param in defaults:
        session.params[param] = session.config.get(param, game_cfg.get(param, defaults[param]))
    reps = session.config.get("repetitions", game_cfg.get("repetitions"))
    if reps is not None:
...
def gen_puzzle(player: Player) -> Question:
    """Pick a random sequence question, avoiding repeats when possible."""
    questions = load_questions()
    used = {q.text for q in Question.filter(player=player)}
...
def get_cur_puzzle(player: Player):
    puzzles = Question.filter(player=player, iteration=player.iteration)
    if puzzles:
        [puzzle] = puzzles
        return puzzle


def enc_puzzle(puzzle: Question):
    return dict(question=puzzle.text)
