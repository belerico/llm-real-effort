...
doc = """
Count digits in a character grid (image-based, single player).
"""


class C(BaseConstants):
    NAME_IN_URL = "count_numbers"
...
def creating_session(subsession: Subsession):
    ...
    game_cfg = get_game_config("count_numbers")
    defaults = dict(
        retry_delay=1.0,
        puzzle_delay=1.0,
...
def gen_puzzle(player: Player) -> Puzzle:
    ...
    counted = params["counted_char"]
    # Build ignored_chars from all digits except the counted char
    ignored = "".join(d for d in "0123456789" if d != counted)
    fields = task_matrix.generate_puzzle_fields(
