...
doc = """
Encoding/Encrypting Words into Numbers (decoding task).
A letter-digit mapping is shown as an image; the participant must decode
a numeric string back into the corresponding letters.
"""
...
def creating_session(subsession: Subsession):
    ...
    game_cfg = get_game_config("task_decoding")
    defaults = dict(
        retry_delay=1.0,
        puzzle_delay=1.0,
...
def gen_puzzle(player: Player) -> Puzzle:
    ...
    fields = task_decoding.generate_puzzle_fields(
        word_length=player.session.params["word_length"],
    )
    player.iteration += 1
...
def enc_puzzle(puzzle: Puzzle):
    image = task_decoding.render_image(puzzle)
...
def play_game(player: Player, msg: dict):
    ...
    if msg_type == "answer":
        ...
        current.is_correct = task_decoding.is_correct(answer, current)
        current.response_timestamp = now
        current.attempts += 1
...
class Game(Page):
    ...
    def vars_for_template(player: Player):
        ...
        return dict(
            ...
            input_type=task_decoding.INPUT_TYPE,
            placeholder=task_decoding.INPUT_HINT,
