...
from . import counting_zeros
from .image_utils import encode_image

doc = """
Count zeros in a paragraph of 1s and 0s (image-based, single player).
"""
...
def creating_session(subsession: Subsession):
    ...
    game_cfg = get_game_config("counting_zeros")
    defaults = dict(
        retry_delay=1.0,
        puzzle_delay=1.0,
...
def gen_puzzle(player: Player) -> Puzzle:
    ...
    fields = counting_zeros.generate_puzzle_fields(
        rows=params["rows"],
        cols=params["cols"],
        min_zeros=params["min_zeros"],
        max_zeros=params["max_zeros"],
    )
    player.iteration += 1
...
def enc_puzzle(puzzle: Puzzle):
    image = counting_zeros.render_image(puzzle)
...
def play_game(player: Player, msg: dict):
    ...
    if msg_type == "answer":
        ...
        current.is_correct = counting_zeros.is_correct(answer, current)
        current.response_timestamp = now
        current.attempts += 1
...
class Game(Page):
    ...
    def vars_for_template(player: Player):
        ...
        return dict(
            ...
            input_type=counting_zeros.INPUT_TYPE,
            placeholder=counting_zeros.INPUT_HINT,
