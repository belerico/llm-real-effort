...
doc = """
Add numbers (image-based, single player).
"""
...
def creating_session(subsession: Subsession):
    ...
    game_cfg = get_game_config("add_numbers")
    defaults = dict(
        retry_delay=1.0,
        puzzle_delay=1.0,
    ...
    # Derive min_num / max_num from max_digits
    md = int(session.params["max_digits"])
    session.params["min_num"] = 10 ** (md - 1) if md > 1 else 1
    session.params["max_num"] = 10**md - 1
...
def gen_puzzle(player: Player) -> Puzzle:
    ...
    fields = task_addition.generate_puzzle_fields(
        num_addends=params["num_addends"],
...
def enc_puzzle(puzzle: Puzzle):
    image = task_addition.render_image(puzzle)
...
def play_game(player: Player, msg: dict):
    ...
    if msg_type == "answer":
        ...
        current.is_correct = task_addition.is_correct(answer, current)
        current.response_timestamp = now
        current.attempts += 1
...
class Game(Page):
    ...
    def vars_for_template(player: Player):
        ...
        return dict(
            ...
            input_type=task_addition.INPUT_TYPE,
            placeholder=task_addition.input_hint(params["num_addends"]),
            num_addends=params["num_addends"],
