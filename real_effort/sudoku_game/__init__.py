...
doc = """
Solve a 6x6 Sudoku puzzle (image-based, single player).
"""
...
def creating_session(subsession: Subsession):
    ...
    game_cfg = get_game_config("sudoku_game")
    defaults = dict(
        retry_delay=1.0,
        puzzle_delay=1.0,
...
def gen_puzzle(player: Player) -> Puzzle:
    ...
    fields = sudoku_puzzle.generate_puzzle_fields(
        difficulty=player.session.params["difficulty"],
    )
    player.iteration += 1
...
def enc_puzzle(puzzle: Puzzle):
    image = sudoku_puzzle.render_image(puzzle)
...
def play_game(player: Player, msg: dict):
    ...
    if msg_type == "answer":
        ...
        current.is_correct = sudoku_puzzle.is_correct(answer, current)
        current.response_timestamp = now
        current.attempts += 1
...
class Game(Page):
    ...
    def vars_for_template(player: Player):
        ...
        return dict(
            ...
            input_type=sudoku_puzzle.INPUT_TYPE,
            placeholder=sudoku_puzzle.INPUT_HINT,
