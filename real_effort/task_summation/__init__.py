...
from . import task_summation
from .image_utils import encode_image

doc = """
Summation grid task. Player sees a 3x3 grid of decimal numbers and must
find the two numbers that sum to 10.0. Uses WebSocket live_method with
a Puzzle ExtraModel.
"""


class C(BaseConstants):
    NAME_IN_URL = "summation"
...
def creating_session(subsession: Subsession):
    ...
    game_cfg = get_game_config("task_summation")
    defaults = dict(
        retry_delay=1.0,
        puzzle_delay=1.0,
...
def gen_puzzle(player: Player) -> Puzzle:
    ...
    fields = task_summation.generate_puzzle_fields(
...
def enc_puzzle(puzzle: Puzzle):
    image = task_summation.render_image(puzzle)
...
def play_game(player: Player, msg: dict):
    ...
    if msg_type == "answer":
        ...
        current.is_correct = task_summation.is_correct(
            answer, current, target_sum=params["target_sum"]
        )
        current.response_timestamp = now
        current.attempts += 1
...
class Game(Page):
    ...
    def vars_for_template(player: Player):
        ...
        return dict(
            ...
            input_type=task_summation.INPUT_TYPE,
            placeholder=task_summation.input_hint(target),
            target_sum=f"{target:g}",
